...
for(indx in 1:length(stk_data_filtered)){
  for(survey in 1:length(stk_data_filtered[[indx]])){
    ...
    sa <- sa_data %>%
      ...
      mutate("Spreading Area (scaled)" = `Spreading Area`/max(`Spreading Area`),
             Quarter = paste(as.character(sort(unique(sa_data$Quarter))), collapse = ", ")) %>% # add quarters
      relocate(Year, Quarter)
    
    ### Equivalent Area 
    print(noquote("Equivalent Area"))
    ea <- sa_data %>%
      group_by(Year) %>%
      summarise("Equivalent Area" = equivalentarea(TotalNo_Dur)) %>%
      mutate("Equivalent Area (scaled)" = `Spreading Area`/max(`Spreading Area`),
             Quarter = paste(as.character(sort(unique(sa_data$Quarter))), collapse = ", ")) %>% # add quarters
      relocate(Year, Quarter)
