...
################################################################################
    ...
    colnames(si)[colnames(si) == "PosAreaR"] <- "Positve Area (Rectangle)"
    si <- si %>%
      mutate(SurveyIndex = names(stk_data_filtered[indx]),
             Survey = names(stk_data_filtered[[indx]][survey]),
             StockID = stk.chr) %>%
      relocate(StockID, SurveyIndex, Survey, Year, Quarter, `Gini Index`, D95, `Positive Area (Haul)`, `Positve Area (Rectangle)`, SPI)
...
" | Survey Index: ", names(stk_data_filtered[indx]), " | Survey: ", names(stk_data_filtered[[indx]][survey]), " (", min(yrs), "-", max(yrs), ", Q", qrs_text, ")")) +
