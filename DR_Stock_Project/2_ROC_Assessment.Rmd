...
################################################################################
    ...
    ## merge
    si <- Reduce(function(x, y) merge(x, y, by = c("Year", "Quarter")), list(Gini.index, D95, np.hauls, np.rects, spi))
    ## rename some columns
    colnames(si)[colnames(si) == "PosAreaH"] <- "Positive Area (Haul)"
    colnames(si)[colnames(si) == "PosAreaR"] <- "Positive Area (Rectangle)"
    si <- si %>%
      mutate(SurveyIndex = names(stk_data_filtered[indx]),
             Survey = names(stk_data_filtered[[indx]][survey]),
             StockID = stk.chr) %>%
      relocate(StockID, SurveyIndex, Survey, Year, Quarter, `Gini Index`, D95, `Positive Area (Haul)`, `Positive Area (Rectangle)`, SPI)
  ...
  if(dir.exists(paste0(si.save.path, "/SpatIndPlot")) == FALSE){
    ...
    si_long$`Spatial Indicator` <- factor(si_long$`Spatial Indicator`, levels = c("Gini Index", "D95", "Positive Area (Haul)", "Positive Area (Rectangle)", "SPI")) # factor & relocate
    qrs_text <- paste(as.character(sort(qrs)), collapse = ", ")
    group_colours <- c(`Gini Index` = 2, D95 = 3, `Positive Area (Haul)` = 4, `Positive Area (Rectangle)` = 5, SPI = 6) # same colours as ROC and TSS
    ...
    si_plot <- ggplot() +
      ...
      scale_x_continuous(limits = c(min(si_long$Year), max(si_long$Year)), breaks = seq(min(si_long$Year), max(si_long$Year), 2)) +
      # Theme
...
for(IndexFolder in list.files(si.stk.path)){
  ...
  for(SurveyFolder in list.files(paste0(si.stk.path, IndexFolder, "/"))){
    print(noquote(paste0("Survey: ", SurveyFolder)))
      ...
      geom_rect(data = posyrsdf, inherit.aes = F, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = posyrsgroup), fill = "grey", alpha=0.1) +
      # Axes breaks
      #coord_cartesian(ylim = c(0,1)) +
      scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.1)) +
...
```{r}
auctss <- data.frame("Survey Index" = NA, "Survey" = NA, "Years" = NA, "Quarter" = NA, "Spatial.Indicator" = NA, "TSS" = NA, "AUC" = NA)
for(IndexFolder in list.files(si.stk.path)){
  message(paste0("Survey Index: ", IndexFolder))
  for(SurveyFolder in list.files(paste0(si.stk.path, IndexFolder, "/"))){
    print(noquote(paste0("Survey: ", SurveyFolder)))
      ...
      spatindcols <- colnames(si[match("Gini Index", colnames(si)):length(colnames(si))])
      outputs <- data.frame("Survey Index" = IndexFolder,
                              "Survey" = SurveyFolder,
                              "Years" = paste0(min(yrs), "-", max(yrs)),
                              "Quarter" = qrs,
...
# Total TSS across all surveys
ggplot() +
  geom_col(data = auctss, aes(y = TSS, x = Spatial.Indicator, fill = Spatial.Indicator)) +
  guides(fill = guide_legend(byrow = TRUE)) + # to enable vertical spacing between key glyphs
  scale_fill_manual(values = spat_colours, guide = "legend") # custom colours matching ROC colours

# Total AUC for each survey withiin each index
ggplot() +
  geom_col(data = auctss, aes(y = AUC, x = Spatial.Indicator, fill = Spatial.Indicator)) +
  guides(fill = guide_legend(byrow = TRUE)) + # to enable vertical spacing between key glyphs
  scale_fill_manual(values = spat_colours, guide = "legend") + # custom colours matching ROC colours
  facet_grid(Survey.Index ~ Survey)

# Total TSS for each survey withiin each index
ggplot() +
  geom_col(data = auctss, aes(y = TSS, x = Spatial.Indicator, fill = Spatial.Indicator), colour = "black") +
  guides(fill = guide_legend(byrow = TRUE)) + # to enable vertical spacing between key glyphs
  scale_fill_manual(values = spat_colours, guide = "legend") + # custom colours matching ROC colours
  facet_grid(Survey.Index ~ Survey) +
  # Theme
