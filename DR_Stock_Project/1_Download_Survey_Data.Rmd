...
```{r}
source("ibts_funs.R") # for dealing with datras data
```

# ICES Divisions and Rectangles 
```{r ices.rect, echo = T, eval = F}
### ICES Rectangles
ices_rect2 <- readOGR(dsn = "ICES Rect/ICES_StatRec_mapto_ICES_Areas (1)", layer = "StatRec_map_Areas_Full_20170124")
ices_rect <- makeReadableFortify(ices_rect2)

ices_rect2 <- readOGR(dsn = "ICES Rect/ICES_rectangles", layer = "ICES_Statistical_Rectangles_Eco")
ices_rect2 <- makeReadableFortify(ices_rect2)


rm(ices_rect2)
```
```{r, save.rect, echo = F, eval = F}
save(ices_rect, file = "C:/Users/pk02/OneDrive - CEFAS/Projects/C8503B/PhD/DATRAS/Spatial Indicator R Project/ICES Rect/ices_rect.rds")
```

```{r ices.divs, echo = T, eval = F}
### ICES Divisions
ices_divs2 <- readOGR(dsn = "ICES Divs/ICES_areas", layer = "ICES_Areas_20160601_cut_dense_3857")
ices_divs2 <- readOGR(dsn = "ICES North Sea Divisions", layer = "ICES_Divisons_CodHadWhgPok")
ices_divs <- makeReadableFortify(ices_divs2)

ices_divs <- makeReadableFortify(ices_divs2)
ices_divs$long <- ices_divs$long/100000
ices_divs$lat <- ices_divs$lat/1000000


rm(ices_divs2)
```
```{r save.divs, eval = F, echo = F}
save(ices_divs, file = "C:/Users/pk02/OneDrive - CEFAS/Projects/C8503B/PhD/DATRAS/Spatial Indicator R Project/ICES Divs/ices_divs.rds")
```
...
```{r load.nsibts, cache = F, echo=FALSE, eval=TRUE}
...
```{r process.nsibts}
# changing name from data.list to ibts.data will mean I need to change 
# things later on and throughout the code
ns_ibts.data <- prepsurveydata(ns_ibts.hh, ns_ibts.hl, ns_ibts.ca, ices_rect)
```

```{r save.nsibts.list, echo=FALSE, eval=FALSE}
save(ns_ibts.data, file = "Survey Data/NS-IBTS/ns_ibts.data.rda")
```

```{r load.nsibts.list, echo = F}
load(paste0(surveydata.path, "NS-IBTS/ns_ibts.data.rda"))
```
...
```{r save.data.list, echo=FALSE, eval=FALSE}
### Save
...
```{r load.data, cache = F, echo=FALSE, eval=TRUE}
### Load
load(paste0(surveydata.path, "BTS/bts_isis.hh.rda"))
load(paste0(surveydata.path, "BTS/bts_isis.hl.rda"))
load(paste0(surveydata.path, "BTS/bts_isis.ca.rda"))
```
